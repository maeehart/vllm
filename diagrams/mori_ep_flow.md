# MORI-EP Expert Parallelism Flow Diagram

## Overview

MORI-EP (Multi-GPU Optimized Routing Interface for Expert Parallelism) handles
the All-to-All communication needed when experts are distributed across GPUs.

---

## 🔑 EXPERT ID FLOW - INVESTIGATION IN PROGRESS (Jan 2026)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                 EXPERT ID FLOW (CURRENT INVESTIGATION)                      │
│                                                                             │
│  DeepSeek-R1: 256 experts (global IDs 0-255) across 8 GPUs                 │
│               32 experts per GPU (local IDs 0-31)                          │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ STEP 1: Router produces GLOBAL expert IDs                           │   │
│  │                                                                     │   │
│  │   topk_ids = [70, 134, 5, 200, 80, 95, 10, 150]  ← GLOBAL (0-255)   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ STEP 2: MORI dispatch sends tokens to correct ranks                 │   │
│  │                                                                     │   │
│  │   Token with expert 70 → sent to Rank 2 (owns experts 64-95)        │   │
│  │   MORI copies FULL tokenIndices to output (all 8 expert IDs!)       │   │
│  │   recv_topk_ids = [70, 134, 5, 200, 80, 95, 10, 150]                │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ STEP 3: Convert GLOBAL → LOCAL IDs                     ← TRYING NOW │   │
│  │                                                                     │   │
│  │   expert_topk_ids = recv_topk_ids - rank_expert_offset              │   │
│  │   On Rank 2 (offset=64):                                            │   │
│  │     [70, 134, 5, 200, 80, 95, 10, 150] - 64 =                       │   │
│  │     [6, 70, -59, 136, 16, 31, -54, 86]                              │   │
│  │                                                                     │   │
│  │   Valid local IDs: 6, 16, 31 (experts 70, 80, 95)                   │   │
│  │   Invalid: 70, -59, 136, -54, 86 (should be filtered by expert_map) │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                              ↓                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ STEP 4: AITER uses expert_map to filter local experts               │   │
│  │                                                                     │   │
│  │   expert_map on Rank 2:                                             │   │
│  │     expert_map[0..31] = 0..31 (valid local)                         │   │
│  │     expert_map[32..255] = -1 (out of range, skip)                   │   │
│  │                                                                     │   │
│  │   After filtering: only process experts 6, 16, 31                   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ⚠️ POTENTIAL ISSUE: expert_map expects GLOBAL IDs, not local!             │
│     If expert_map[70] = 6, then passing local ID 6 gives expert_map[6] = 6 │
│     This might actually work OR might cause duplicate processing!          │
└─────────────────────────────────────────────────────────────────────────────┘
```

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                        MORI-EP EXPERT PARALLELISM FLOW                          │
│                                                                                 │
│  DeepSeek-R1: 256 experts ÷ 8 GPUs = 32 experts/GPU, topk=8                    │
└─────────────────────────────────────────────────────────────────────────────────┘

Input: hidden_states [M, H]  (M tokens, H=7168 hidden dim)
       topk_ids [M, K]       (K=8 selected experts per token)
       topk_weights [M, K]   (routing weights)

                    ╔═══════════════════════════════════╗
                    ║   GPU 0        GPU 1    ...  GPU 7║
                    ║  exp 0-31    exp 32-63     exp 224-255
                    ╚═══════════════════════════════════╝
```

---

## Full Forward Pass Through FusedMoE Layer

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                    FusedMoE.forward() [layer.py:845-920]                     │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                     Router.forward() [router/__init__.py]                    │
│                                                                              │
│  Input:  hidden_states [M, H]                                                │
│  Output: topk_ids [M, K], topk_weights [M, K]                               │
│                                                                              │
│  For DeepSeek-R1:                                                            │
│  - Uses grouped_topk with e_score_correction_bias                            │
│  - On ROCm: calls rocm_aiter_grouped_topk()                                  │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│               FusedMoEModularKernel.forward() [modular_kernel.py:1171]       │
│                                                                              │
│  1. _prepare()  → Dispatch tokens to expert owners                           │
│  2. _fused_experts() → Compute expert outputs                                │
│  3. _finalize() → Combine results back                                       │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

## Step 1: PREPARE (Dispatch Phase)

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                      _prepare() [modular_kernel.py:938-1010]                 │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│            MoriPrepareAndFinalize.prepare_async() [mori_prepare_finalize.py] │
│                                                                              │
│  def prepare_async(                                                          │
│      self,                                                                   │
│      a1: torch.Tensor,           # [M, H] input activations                  │
│      topk_weights: torch.Tensor, # [M, K] router weights                     │
│      topk_ids: torch.Tensor,     # [M, K] selected expert IDs (global)       │
│      num_experts: int,           # 256 total experts                         │
│      expert_map: torch.Tensor,   # NOT used with MORI                        │
│      apply_router_weight_on_input: bool,                                     │
│      quant_config: FusedMoEQuantConfig,                                      │
│  ) -> Callable[[], PrepareResultType]:                                       │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ Step 1: Optional weight application (for topk=1)                       │  │
│  │   if apply_router_weight_on_input:                                     │  │
│  │       a1 = a1 * topk_weights.to(a1.dtype)                             │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ Step 2: Quantization Strategy                                          │  │
│  │                                                                        │  │
│  │   Strategy A (FP8 dispatch): use_fp8_dispatch=True                     │  │
│  │   - Quantize BEFORE dispatch for 2x bandwidth savings                  │  │
│  │   - a1q, a1q_scale = moe_kernel_quantize_input(a1, ...)               │  │
│  │                                                                        │  │
│  │   Strategy B (BF16 dispatch): use_fp8_dispatch=False (DEFAULT)         │  │
│  │   - Dispatch BF16, quantize AFTER receive                              │  │
│  │   - a1q = a1, a1q_scale = None                                        │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ Step 3: Call _do_dispatch()                                            │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│              _do_dispatch() [mori_prepare_finalize.py:199-240]               │
│                                                                              │
│  # MORI dispatch - send tokens to expert owners                              │
│  dispatch_result = self.ep_op.dispatch(                                      │
│      input=tokens,      # [M, H] or [M_quant, H] if FP8                     │
│      weights=topk_weights,  # [M, K]                                        │
│      scales=token_scales,   # None or [M, scale_dim] for FP8                │
│      indices=topk_ids.to(torch.int32),  # [M, K] global expert IDs          │
│  )                                                                           │
│                                                                              │
│  Returns: (recv_x, recv_weights, recv_scale, recv_topk_ids, recv_src_pos)   │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│        MORI launch_dispatch() [mori/cpp/...]                                 │
│                                                                              │
│  ALL-TO-ALL COMMUNICATION:                                                   │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │                                                                        │  │
│  │   GPU 0: tokens for experts 0-31     ──────┐                          │  │
│  │   GPU 1: tokens for experts 32-63    ──────┼──► All-to-All via XGMI   │  │
│  │   ...                                      │                          │  │
│  │   GPU 7: tokens for experts 224-255  ──────┘                          │  │
│  │                                                                        │  │
│  │   After dispatch, each GPU has ALL tokens routed to its local experts │  │
│  │                                                                        │  │
│  │   GPU 0 receives: tokens from ALL GPUs that selected experts 0-31     │  │
│  │   GPU 1 receives: tokens from ALL GPUs that selected experts 32-63    │  │
│  │   ...                                                                  │  │
│  │                                                                        │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  Output buffers are FIXED SIZE: [max_num_tokens, hidden_dim]                 │
│  (This is important - causes size mismatch issues during CUDA graph)         │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│              _receiver() [mori_prepare_finalize.py:242-315]                  │
│                                                                              │
│  # Unpack dispatch result                                                    │
│  recv_x_full = dispatch_result[0]      # [MAX_RECV, H] FIXED SIZE buffer   │
│  recv_weights_full = dispatch_result[1]# [MAX_RECV, K] weights             │
│  recv_scale_full = dispatch_result[2]  # [MAX_RECV, scale_dim] or None     │
│  recv_topk_ids = dispatch_result[3]    # [MAX_RECV, K] GLOBAL expert IDs   │
│  total_recv_tokens = dispatch_result[4]# Scalar: actual valid token count  │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ ⚠️ CRITICAL: Slice to valid tokens only!                               │  │
│  │                                                                        │  │
│  │ MORI returns FIXED-SIZE buffers [max_num_tokens, ...]                  │  │
│  │ Only positions 0..total_recv_tokens-1 contain valid data!              │  │
│  │                                                                        │  │
│  │ num_valid = total_recv_tokens.item()                                   │  │
│  │ recv_x = recv_x_full[:num_valid]                                       │  │
│  │ recv_weights = recv_weights_full[:num_valid]                           │  │
│  │ recv_topk_ids = recv_topk_ids[:num_valid]                              │  │
│  │                                                                        │  │
│  │ NOTE: .item() breaks CUDA graph capture! Use --enforce-eager for now   │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ ✅ Expert IDs: Keep GLOBAL, pass to AITER unchanged                    │  │
│  │                                                                        │  │
│  │ MORI dispatch returns the ORIGINAL global expert IDs (0-255)           │  │
│  │ from the router. It just copies tokenIndices to shmemOutIndices.       │  │
│  │                                                                        │  │
│  │ expert_topk_ids = recv_topk_ids  # Keep as-is! (GLOBAL IDs)            │  │
│  │                                                                        │  │
│  │ AITER expects GLOBAL IDs when expert_map is provided:                  │  │
│  │   expert_map[global_id] → local_id (or -1 if not on this rank)         │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ Strategy B post-dispatch quantization (if not FP8 dispatch)            │  │
│  │                                                                        │  │
│  │ if not quant_config.is_block_quantized:                                │  │
│  │     expert_x, expert_x_scale = moe_kernel_quantize_input(expert_x, ...)│  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  Return: (expert_x, expert_x_scale, None, expert_topk_ids, recv_weights)    │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

## Step 2: FUSED EXPERTS (Computation Phase)

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                  _fused_experts() [modular_kernel.py:1012-1100]              │
│                                                                              │
│  # Get problem dimensions                                                    │
│  M_full = a1q.size(0)  # Number of tokens received by this rank             │
│                                                                              │
│  # Allocate output buffer                                                    │
│  fused_out = torch.empty_like(a1q, dtype=in_dtype)  # [N_recv, H]           │
│                                                                              │
│  # Call AiterExperts.apply()                                                 │
│  self.fused_experts.apply(...)                                               │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                AiterExperts.apply() [rocm_aiter_fused_moe.py:310-345]        │
│                                                                              │
│  def apply(                                                                  │
│      self,                                                                   │
│      output: torch.Tensor,          # [N_recv, H] output buffer             │
│      hidden_states: torch.Tensor,   # [N_recv, H] received tokens           │
│      w1: torch.Tensor,              # [32, I, H] gate/up weights            │
│      w2: torch.Tensor,              # [32, H, I] down weights               │
│      topk_weights: torch.Tensor,    # [N_recv, K]                           │
│      topk_ids: torch.Tensor,        # [N_recv, K] now LOCAL expert IDs      │
│      activation: str,               # "silu"                                 │
│      global_num_experts: int,       # 256                                   │
│      expert_map: torch.Tensor,      # Maps global→local expert IDs          │
│      ...                                                                     │
│  ):                                                                          │
│      result = rocm_aiter_fused_experts(...)                                  │
│      output.copy_(result)                                                    │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│           rocm_aiter_fused_experts() [rocm_aiter_fused_moe.py:159-225]       │
│                                                                              │
│  # AITER kernel handles all 32 local experts                                 │
│  rocm_aiter_ops.fused_moe(                                                   │
│      hidden_states,    # [N_recv, H]                                        │
│      w1,               # [32, I, H] local expert gate/up weights            │
│      w2,               # [32, H, I] local expert down weights               │
│      topk_weights,     # [N_recv, K]                                        │
│      topk_ids,         # [N_recv, K]                                        │
│      expert_mask=expert_map,                                                 │
│      quant_method=...,                                                       │
│      activation_method=SILU,                                                 │
│      ...                                                                     │
│  )                                                                           │
│                                                                              │
│  Internally for each token:                                                  │
│  1. Route to selected experts (up to K=8 per token)                          │
│  2. For each selected expert e in token's topk_ids:                          │
│     - x1 = x @ w1[e]  # [H] → [I]  gate/up projection                       │
│     - x1 = SiLU(x1[:I/2]) * x1[I/2:]  # SwiGLU activation                   │
│     - x2 = x1 @ w2[e]  # [I/2] → [H]  down projection                       │
│  3. Weighted sum: out = sum(topk_weights[i] * expert_out[i])                │
│                                                                              │
│  Returns: [N_recv, H] expert outputs                                         │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

## Step 3: FINALIZE (Combine Phase)

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                   _finalize() [modular_kernel.py:1102-1168]                  │
│                                                                              │
│  # Call finalize_async and wait for result                                   │
│  receiver = self.prepare_finalize.finalize_async(...)                        │
│  receiver()  # Execute combine and copy result                               │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│          MoriPrepareAndFinalize._finalize_impl() [mori_prepare_finalize.py]  │
│                                                                              │
│  def _finalize_impl(                                                         │
│      self,                                                                   │
│      output: torch.Tensor,           # [M, H] final output buffer           │
│      fused_expert_output: torch.Tensor,  # [N_recv, H] expert results       │
│      topk_weights: torch.Tensor,     # [M, K] original weights              │
│      topk_ids: torch.Tensor,         # [M, K] original expert IDs           │
│      apply_router_weight_on_input: bool,                                     │
│      weight_and_reduce_impl: TopKWeightAndReduce,                            │
│      do_async: bool,                                                         │
│  ):                                                                          │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ Apply weights if using delegate reducer                                │  │
│  │                                                                        │  │
│  │ if isinstance(weight_and_reduce_impl, TopKWeightAndReduceDelegate):    │  │
│  │     weight_and_reduce_impl = TopKWeightAndReduceContiguous()           │  │
│  │ fused_expert_output = weight_and_reduce_impl.apply(...)               │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ MORI combine - returns results to original token owners                │  │
│  │                                                                        │  │
│  │ combine_result = self.ep_op.combine(                                   │  │
│  │     input=fused_expert_output,  # [N_recv, H] BF16                     │  │
│  │     weights=topk_weights if not apply_router_weight_on_input else None,│  │
│  │     indices=topk_ids.to(torch.int32),                                  │  │
│  │     call_reset=True,  # Reset for next iteration                       │  │
│  │ )                                                                      │  │
│  │                                                                        │  │
│  │ combined_x = combine_result[0]  # [max_num_tokens, H] FIXED SIZE!      │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ ⚠️  CRITICAL FIX: Slice to actual batch size                           │  │
│  │                                                                        │  │
│  │ # MORI combine returns FIXED-SIZE buffer [max_num_tokens, hidden_dim]  │  │
│  │ # but actual batch may be smaller (especially during CUDA graph)       │  │
│  │                                                                        │  │
│  │ num_tokens = output.shape[0]                                           │  │
│  │ if combined_x.shape[0] != num_tokens:                                  │  │
│  │     combined_x = combined_x[:num_tokens]  # SLICE TO ACTUAL SIZE       │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                    │                                         │
│                                    ▼                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │ Copy to output                                                         │  │
│  │                                                                        │  │
│  │ output.copy_(combined_x, non_blocking=True)                            │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│        MORI launch_combine() [mori/cpp/...]                                  │
│                                                                              │
│  ALL-TO-ALL COMMUNICATION (REVERSE):                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │                                                                        │  │
│  │   GPU 0: expert outputs for experts 0-31   ──────┐                    │  │
│  │   GPU 1: expert outputs for experts 32-63  ──────┼──► All-to-All      │  │
│  │   ...                                            │    via XGMI        │  │
│  │   GPU 7: expert outputs for experts 224-255 ─────┘                    │  │
│  │                                                                        │  │
│  │   After combine, each GPU has its ORIGINAL tokens back                 │  │
│  │   with expert computation results                                      │  │
│  │                                                                        │  │
│  │   GPU 0 receives: expert outputs for tokens it originally owned       │  │
│  │   GPU 1 receives: expert outputs for tokens it originally owned       │  │
│  │   ...                                                                  │  │
│  │                                                                        │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

## Memory Layout and Buffer Sizes

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                          MORI BUFFER ALLOCATION                              │
│                                                                              │
│  Created in create_mori_ep_op() [mori_utils.py]                             │
│                                                                              │
│  MoriEpConfig:                                                               │
│  ├── max_num_tokens = 8192 (from scheduler_config.max_num_batched_tokens)   │
│  ├── hidden_dim = 7168                                                       │
│  ├── topk = 8                                                                │
│  ├── num_experts = 256                                                       │
│  ├── num_experts_per_rank = 32                                              │
│  └── world_size = 8                                                          │
│                                                                              │
│  MORI Symmetric Heap:                                                        │
│  ├── MORI_SHMEM_HEAP_SIZE=12G (shared across all 8 GPUs!)                   │
│  ├── Per-rank allocation: ~1.5GB                                            │
│  │                                                                           │
│  │  Required per rank for dispatch:                                          │
│  │  - Input buffer: max_tokens × hidden_dim × 2 bytes (BF16)                │
│  │  - Weight buffer: max_tokens × topk × 4 bytes (FP32)                     │
│  │  - Index buffer: max_tokens × topk × 4 bytes (INT32)                     │
│  │  - Metadata: ~misc overhead                                               │
│  │                                                                           │
│  │  Estimated: 8192 × 7168 × 2 + 8192 × 8 × 4 × 2 ≈ 117MB + 0.5MB = ~120MB │
│  │  But MORI allocates for worst-case all-to-all: ~940MB per rank           │
│  │  Total: 940MB × 8 = 7.5GB minimum → set to 12GB for safety               │
│  └──────────────────────────────────────────────────────────────────────────│
│                                                                              │
│  ⚠️  MORI OUTPUT BUFFERS ARE FIXED SIZE!                                     │
│  ├── dispatch returns: [max_num_tokens, hidden_dim]                         │
│  ├── combine returns: [max_num_tokens, hidden_dim]                          │
│  └── Actual batch size may be smaller → MUST SLICE!                         │
└──────────────────────────────────────────────────────────────────────────────┘
```

---

## Potential Performance Issues

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                     🔍 POTENTIAL SLOWNESS CAUSES                             │
└──────────────────────────────────────────────────────────────────────────────┘

1. DOUBLE WEIGHT APPLICATION?
   ─────────────────────────────────────────────────────────────────────────────
   Check if weights are being applied TWICE:
   
   a) In _finalize_impl():
      fused_expert_output = weight_and_reduce_impl.apply(...)  # Applies weights?
      
   b) In MORI combine():
      combine_result = self.ep_op.combine(
          input=fused_expert_output,
          weights=topk_weights,  # <-- Passes weights again!
          ...
      )
   
   ⚠️  If both apply weights, we have double computation!

2. TOPKWEIGHTANDREDUCE MISMATCH
   ─────────────────────────────────────────────────────────────────────────────
   In finalize, we check for TopKWeightAndReduceDelegate and swap to Contiguous:
   
   if isinstance(weight_and_reduce_impl, TopKWeightAndReduceDelegate):
       weight_and_reduce_impl = TopKWeightAndReduceContiguous()
   
   But AiterExperts returns TopKWeightAndReduceNoOP:
   
   class AiterExperts:
       def finalize_weight_and_reduce_impl(self) -> mk.TopKWeightAndReduce:
           return TopKWeightAndReduceNoOP()  # <-- NoOp!
   
   So weight_and_reduce_impl.apply() does NOTHING!
   
   Then MORI combine() receives weights and applies them.
   This seems correct - but verify MORI actually uses the weights.

3. EXTRA COPY OPERATIONS
   ─────────────────────────────────────────────────────────────────────────────
   In AiterExperts.apply():
      result = rocm_aiter_fused_experts(...)
      output.copy_(result)  # Copy #1
   
   In _finalize_impl():
      output.copy_(combined_x, non_blocking=True)  # Copy #2
   
   ⚠️  Two copies per MoE layer × 61 MoE layers = 122 extra copies!

4. SYNCHRONIZATION POINTS?
   ─────────────────────────────────────────────────────────────────────────────
   Check if there are hidden synchronization points:
   
   - MORI dispatch() - likely async
   - MORI combine(call_reset=True) - does reset cause sync?
   - output.copy_(combined_x, non_blocking=True) - async copy
   
   ⚠️  call_reset=True might force synchronization!

5. ✅ EXPERT ID HANDLING (FIXED Jan 2026)
   ─────────────────────────────────────────────────────────────────────────────
   Previous BUG: We were ADDING rank_expert_offset to global IDs!
   
      expert_topk_ids = recv_topk_ids + rank_expert_offset  # ❌ WRONG!
      # Global ID 70 on rank 2 → 70 + 64 = 134 (out of range!)
   
   FIX: MORI returns GLOBAL IDs (same as router output). Pass unchanged:
   
      expert_topk_ids = recv_topk_ids  # ✅ Keep global IDs (0-255)
      # AITER uses expert_map[70] → 6 (local) internally
   
6. QUANTIZATION STRATEGY
   ─────────────────────────────────────────────────────────────────────────────
   Default use_fp8_dispatch=False → Strategy B (BF16 dispatch)
   
   This means:
   - Dispatch sends BF16 (2 bytes per element)
   - After receive, we quantize to FP8 (if needed for expert compute)
   
   ⚠️  FP8 dispatch would send half the data!
   
7. MORI BUFFER REGISTRATION
   ─────────────────────────────────────────────────────────────────────────────
   MORI config has: use_external_inp_buf=True
   
   This might mean input buffers are NOT pre-registered with MORI,
   causing extra memory registration overhead per dispatch.

8. AITER EXPERT COMPUTATION
   ─────────────────────────────────────────────────────────────────────────────
   AiterExperts doesn't use expert_map efficiently with MORI?
   
   MORI returns LOCAL expert IDs (0-31 for each rank)
   But we remap to GLOBAL and pass expert_map to AITER.
   
   Does AITER then re-map global → local internally?
   That's redundant!
```

---

## Recommendations for Investigation

```
1. Add timing instrumentation:
   - Time MORI dispatch()
   - Time AITER fused_moe()
   - Time MORI combine()
   - Time expert ID remapping
   - Time weight application

2. Check weight application:
   - Does MORI combine() actually use the weights parameter?
   - Is weight_and_reduce_impl doing anything?

3. Try FP8 dispatch:
   export VLLM_MORI_EP_USE_FP8_DISPATCH=1
   - Should halve dispatch/combine bandwidth

4. Check for unnecessary expert_map usage:
   - If MORI gives local IDs, don't remap to global then back to local

5. Profile MORI reset():
   - Does call_reset=True cause synchronization?

6. Check buffer registration:
   - Is MORI re-registering buffers each time?
```

---

## 🔍 INVESTIGATION RESULTS (Jan 2026)

After analyzing the code, here are the key findings:

### 1. ✅ NO DOUBLE WEIGHT APPLICATION
```
AITER fused_moe:
├── Takes topk_weights as input
├── Applies weights during expert computation: out_i = sum(w_j * expert_j(x))
├── Returns [M_recv, H] with weights already applied
└── That's why it returns TopKWeightAndReduceNoOP

MORI combine:
├── Takes weights as input BUT...
├── Only ACCUMULATES weights separately: shmemCombineOutWeightsMemObj
├── Does NOT multiply tokens by weights
├── Just does All-to-All reduce (sum) of token data
└── We DON'T USE the accumulated weights (combine_result[0] only)

CONCLUSION: Weight application is correct, but we're doing unnecessary work
           by passing weights to MORI (they just get accumulated and discarded)
```

### 2. ⚠️ UNNECESSARY OPERATIONS

```python
# In _finalize_impl:
combine_result = self.ep_op.combine(
    input=fused_expert_output,
    weights=topk_weights if not apply_router_weight_on_input else None,  # ❌ UNNECESSARY
    indices=topk_ids.to(torch.int32),
    call_reset=True,
)
combined_x = combine_result[0]  # We only use tokens, discard weights!
```

**Fix**: Pass `weights=None` since AITER already applied weights:
```python
combine_result = self.ep_op.combine(
    input=fused_expert_output,
    weights=None,  # ✅ AITER already applied weights
    indices=topk_ids.to(torch.int32),
    call_reset=True,
)
```

### 3. ✅ EXPERT ID BUG FIXED

```python
# BEFORE (WRONG):
expert_topk_ids = recv_topk_ids + self.rank_expert_offset  # ❌ 70 → 134

# AFTER (CORRECT):
expert_topk_ids = recv_topk_ids  # ✅ Keep global IDs unchanged
```

MORI's C++ dispatch copies original tokenIndices to output unchanged:
```cpp
// intranode.hpp line 135-137:
args.shmemOutIndicesMemObj[...] = args.tokenIndices[...]  // Original global IDs!
```

AITER expects global IDs when expert_map is provided:
- expert_map[global_id] → local_id (or -1 if not on this rank)
- NO modification needed in our code!

### 4. ⚠️ CUDA GRAPH COMPATIBILITY & BUFFER SLICING

MORI returns FIXED-SIZE buffers:
- dispatch returns: [8192, 7168] but only [0:N_valid] has real data!
- combine returns: [8192, 7168] (fixed)

**Critical Bug Found**: Without slicing, expert kernels compute on garbage data!

```python
# BEFORE (WRONG): Pass full 8192 tokens (most are garbage)
expert_x = recv_x_full  # [8192, H] - positions N+1..8191 are uninitialized!

# AFTER (CORRECT): Slice to valid tokens only
num_valid = total_recv_tokens.item()  # e.g., 512
expert_x = recv_x_full[:num_valid]    # [512, H] - only real data
```

**Trade-off**: Using `.item()` breaks CUDA graph capture. Must use `--enforce-eager`.
For CUDA graph support, need kernel-level masking or pre-registered buffers.

### 5. 🔴 POTENTIAL ROOT CAUSE: Strategy B (BF16 dispatch)

Default: `use_fp8_dispatch = False`

This means:
1. Dispatch sends BF16 (2 bytes/element) over XGMI
2. After receive, quantize to FP8 for expert compute
3. Expert compute in FP8
4. Combine sends BF16 (2 bytes/element) over XGMI

With FP8 dispatch:
1. Quantize BEFORE dispatch (on each GPU locally)
2. Dispatch sends FP8 (1 byte/element) - 2x bandwidth savings
3. Expert compute in FP8
4. Combine sends BF16 (still 2 bytes)

For 70k tokens × 8 experts × 7168 hidden:
- BF16 dispatch: ~4GB per All-to-All
- FP8 dispatch: ~2GB per All-to-All

### 6. 🔴 61 MoE LAYERS × 2 All-to-All = 122 Communications

For 70k input tokens:
- Each All-to-All moves ~4GB (BF16)
- 122 × 4GB = ~488GB total data movement
- Even with XGMI at ~300GB/s, this is ~1.6 seconds just for communication!

---

## Implemented Fixes (Jan 2026)

### ✅ Fix 1: Expert ID Handling (CRITICAL - was causing garbage output!)
```python
# BEFORE (WRONG):
expert_topk_ids = recv_topk_ids + self.rank_expert_offset  # 70 → 134 ❌

# AFTER (CORRECT):
expert_topk_ids = recv_topk_ids  # 70 stays 70, expert_map handles conversion ✅
```

### ✅ Fix 2: Slice Dispatch Buffer to Valid Tokens
```python
# BEFORE (WRONG): Used full 8192-token buffer (garbage data!)
expert_x = recv_x_full  # [8192, H] includes uninitialized garbage

# AFTER (CORRECT): Slice to actual received tokens
num_valid = total_recv_tokens.item()
expert_x = recv_x_full[:num_valid]  # [N_valid, H] only real data
```

### ✅ Fix 3: Don't Pass Weights to MORI Combine
```python
# BEFORE: Unnecessary weight transfer
combine_result = self.ep_op.combine(..., weights=topk_weights, ...)

# AFTER: AITER already applied weights
combine_result = self.ep_op.combine(..., weights=None, ...)
```

### ⚠️ Known Limitation: CUDA Graphs
The `.item()` call in buffer slicing breaks CUDA graph capture.
Must use `--enforce-eager` until kernel-level masking is implemented.

---

## Remaining Optimizations (Future Work)

### Priority 1: Enable FP8 Dispatch
```bash
export VLLM_MORI_EP_USE_FP8_DISPATCH=1
```
Expected: ~2x bandwidth reduction for dispatch (1 byte vs 2 bytes)

### Priority 2: CUDA Graph Support
Need kernel-level support for `num_valid_tokens` parameter to avoid `.item()`.
Or use pre-registered fixed-size buffers with masking in AITER kernel.

---

## Quick Test Commands

```bash
# Test with FP8 dispatch
VLLM_MORI_EP_USE_FP8_DISPATCH=1 \
MORI_SHMEM_HEAP_SIZE=12G \
vllm serve deepseek-ai/DeepSeek-R1 \
  --host localhost --port 8000 \
  --tensor-parallel-size 8 \
  --max-model-len 72000 \
  --trust-remote-code \
  --enable-expert-parallel \
  --all2all-backend mori_ep \
  --kv-cache-dtype fp8
```

---

## Data Flow Summary

```
Input: [M tokens] × [H=7168]

DISPATCH (All-to-All #1):
├── GPU0 sends tokens for experts 0-31 to GPU0 (local)
├── GPU0 sends tokens for experts 32-63 to GPU1
├── ...
└── Each GPU receives ALL tokens routed to its 32 experts

COMPUTE:
├── Each GPU runs AITER on its 32 local experts
├── Input: [N_recv tokens] × [H]
├── Output: [N_recv tokens] × [H]
└── Where N_recv varies per GPU based on routing

COMBINE (All-to-All #2):
├── GPU0 receives expert outputs for its original tokens
├── GPU1 receives expert outputs for its original tokens
├── ...
└── Final: [M tokens] × [H] (original shape)
```

