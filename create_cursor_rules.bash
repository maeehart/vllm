...
# ⚠️ STOP — READ THESE RULES BEFORE EVERY ACTION ⚠️

## 🚫 FORBIDDEN ACTIONS (Never do without explicit permission)
1. **Running benchmarks/servers/long commands** — NEVER run these yourself. Tell user the command.
2. **pip install** — ALWAYS run `pip show <pkg>` first. Editable installs persist.
3. **Reinstall packages** — Ask first: "Package X missing. Reinstall? (was editable)"
4. **Push to upstream** — Only push to YOUR fork
5. **Retry failed approaches** — Check [TRIED] comments, try something DIFFERENT
6. **Give up silently** — Always offer alternatives before abandoning
7. **Ignore `*_markus.*` wrappers** — If wrapper exists, use it. Don't read original tool.

## ✅ ALWAYS DO
1. **Advise, don't run** — For benchmarks/servers/training, say "Please run: <command>" 
2. Check for `*_markus.{sh|py}` wrappers before using repo tools
3. Commit every ~50-100 lines
4. Ask before changing/removing tasks
...
1. Check for `*_markus.*` wrapper first (both `.sh` AND `.py`) → use directly if exists
...
5. **Always use `*_markus.*` tools directly** — don't re-read original tools
...
⚠️ **NEVER reinstall packages without explicit user instruction:**
- Before ANY `pip install`: run `pip show <package>` to check if installed
- Editable installs (`pip install -e .`) PERSIST — do NOT reinstall
