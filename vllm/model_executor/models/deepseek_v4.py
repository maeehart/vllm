...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def _forward_rocm(
        ...
        # ffn_norm is folded into self.ffn.norm_gate (NormGateLinear);
        # ffn() takes the pre-norm activation directly. See _forward_cuda
        # and the ``ffn_norm`` ownership comment in __init__.
        x = self.ffn(x, input_ids)
        x = self.hc_post(x, residual, post, comb)
