...
if TYPE_CHECKING:
    ...
    VLLM_ROCM_AITER_ASM_PA_PRECISION: int = 1
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Whether to use AITER ASM paged attention with shuffle KV cache layout.
    # Provides 20-70% speedup at high concurrency for decode on MI300X/MI325X/MI355X.
    # Requires block_size=16 and FP8 KV cache. By default is disabled.
    "VLLM_ROCM_USE_AITER_ASM_PA": lambda: (
        os.getenv("VLLM_ROCM_USE_AITER_ASM_PA", "False").lower() in ("true", "1")
    ),
    # Precision level for AITER ASM paged attention (0=normal, 1=high, 2=ultra-high).
    # Higher precision reduces numerical errors but may be slightly slower.
    # Only affects FP8 KV cache. Default is 1 (high precision).
    "VLLM_ROCM_AITER_ASM_PA_PRECISION": lambda: int(
        os.getenv("VLLM_ROCM_AITER_ASM_PA_PRECISION", "1")
    ),
    ...
    # These are hand-tuned GCN assembly kernels for MI300X (gfx950)
    # Requires block_size=16 and FP8 KV cache with per-token quantization
    "VLLM_ROCM_USE_AITER_ASM_PA": lambda: (
